...
var initializeVideoJS = function () {
    var play_pause = $('#videoContainer .playButton'),
        container = $('#videoContainer'),
        playList = $("#playlist"),
        playIcon = $('.playButton .playPause'),
        video = $('#video video'),
        videoNative = document.querySelector('#videoContainer video'),
        videoTitle = $('.video-title'),
        play = $('#videoContainer .play'),
        volume = $('.volume .icon'),
        ...
        overlayScreen = $('#videoContainer .overlay'),
        timeState = $('.time'),
        overlayButton = $('#videoContainer .overlay .button'),
        vbtnPrev = $('.vbtn-prev'),
        vbtnNext = $('.vbtn-next'),
    ...
    //videoNative.addEventListener('canplay', videoIsReady);
    videoNative.addEventListener('timeupdate', updateplayer);
    videoNative.addEventListener('click', playVid);
    videoNative.addEventListener('loadstart', function (event) {
        // $(this).addClass('bkg');
        // $(this).attr("poster", "http://iphonewrd.com/img/loading.gif");
        preload.classList.remove('d-none');
        playVid();
    });
    // videoNative.addEventListener('canplay', function (event) {
    //     // $(this).removeClass('bkg');
    //     // $(this).removeAttr("poster");
    //     console.log('hi... canplay')
    //     preload.classList.remove('d-none');
    // });

    // videoNative.addEventListener("progress", function () {
    //     // When buffer is 1 whole video is buffered
    //     console.log('show loader ', videoNative.buffered);
    //     preload.classList.add('d-none');
    //     if (videoNative.buffered.length >= 0) {
    //         if (Math.round(videoNative.buffered.end(0)) / Math.round(videoNative.seekable.end(0)) === 1) {
    //             // Entire video is downloaded
    //             preload.classList.remove('d-none');
    //             console.log('hide show loader');
    //         }
    //     }
    // }, false);
    ...
    progressBar.mousemove(function (evt) {
        timeState.text(getTimeState(evt));
    });
    ...
    volumeIntesity.click(function (evt) {
        changeVolume(evt);
    });
    ...
    playList.click(function (e) {
        ...
        if (target.id != "playlist") {
            if (target.closest(".video") != null) {
                var id = target.closest(".video").id,
                    el = target.closest(".video"),
                    name = el.getAttribute('data-name');
                preload.classList.remove('d-none');
                var w = $(window).width() - ($('.video.playing').offset().left + $('.video.playing').width()),
                    boxW = $('.video.playing').width();
                disableNextPrev(target, id);
    ...
    vbtnNext.click(function () {
        var current = $(playList).find('.playing'),
            nextEl = $(current).next(),
            id = $(nextEl).attr('id'),
            name = $(nextEl).attr('data-name');
        if (id != undefined && name != undefined) {
            preload.classList.remove('d-none');
            disableNextPrev(playList, id);
            higlight(id, currentTrack.toString());
            videoTitle.text(name);
            set(videos[id].source);
            videoNative.addEventListener('canplay', videoIsReady);
    ...
    vbtnPrev.click(function () {
        var current = $(playList).find('.playing'),
            prevEl = $(current).prev(),
            id = $(prevEl).attr('id'),
            name = $(prevEl).attr('data-name');
        if (id != undefined && name != undefined) {
            preload.classList.remove('d-none');
            disableNextPrev(playList, id);
            higlight(id, currentTrack.toString());
    ...
    function setScroll(val){
        var _instance = $('#playlist').overlayScrollbars(),
            _el = document.querySelector('#playlist .video'),
            lH = _el.offsetHeight,
            lW  = _el.offsetWidth;
        if($(window).width() >= 768 ){
            if(val == 'prev'){
                _instance.scroll({ x : "-="+lW+""});
            }else{
                _instance.scroll({ x : "+="+lW+""});
            }
        }else{
            if(val == 'prev'){
                _instance.scroll({ x : "-="+lH+""});
            }else{
                _instance.scroll({ x : "+="+lH+""});
            }
        }
    ...
    function disableNextPrev(list, id) {
        var fId = $(playList).find('.video:first-child').attr('id'),
            lId = $(playList).find('.video:last-child').attr('id');
        if (fId == id) {
            $(vbtnPrev).prop("disabled", true);
            $(vbtnNext).prop("disabled", false);
        } else if (lId == id) {
            $(vbtnNext).prop("disabled", true);
            $(vbtnPrev).prop("disabled", false);
        } else {
            $(vbtnNext).prop("disabled", false);
            $(vbtnPrev).prop("disabled", false);
        }
    ...
    function playVid() {
        if (video.get(0).paused) {
            video.get(0).play();
            playIcon.css({
                'background': 'url(/etc.clientlibs/incredible-india-v2/components/clientlibs/main/resources/video/pause.svg)',
            ...
            video.get(0).pause();
            playIcon.css({
                'background': 'url(/etc.clientlibs/incredible-india-v2/components/clientlibs/main/resources/video/play.svg)',
                'background-size': '100% 100%'
            });
            overlayScreen.show();
    ...
    function updateplayer() {
        var percentage = (videoNative.currentTime / videoNative.duration) * 100;
        progress.css({
            'width': percentage + '%'
        });
        ...
        if (videoNative.ended) {
            var videoLen = $(playList).find('.video').length,
                current = $(playList).find('.playing'),
                nextEl = $(current).next();
            if(videoLen > 0){
                if(nextEl.length != 0){
                    $(nextEl).click();
                    ...
                    overlayScreen.show();
                    overlayButton.css({
                        'background': 'url(/etc.clientlibs/incredible-india-v2/components/clientlibs/main/resources/video/replay.svg)',
                        'background-size': '100% 100%'
                    });
                }
            }

        } else if (videoNative.paused) {
            overlayButton.css({
                'background': 'url(/etc.clientlibs/incredible-india-v2/components/clientlibs/main/resources/video/play-button.svg)',
                'background-size': '100% 100%'
            });
        }
    }
    ...
    function getTimeState(event) {
        ...
        var currentSeconeds = Math.round((mouseX / width) * videoNative.duration);
    ...
    function skip(event) {
        var mouseX = event.pageX - progressBar.offset().left,
            width = progressBar.outerWidth();
        videoNative.currentTime = (mouseX / width) * videoNative.duration;
        updateplayer();
    }

    function getFormatedFullTime() {

        var totalSeconeds = Math.round(videoNative.duration);
    ...
    function getFormatedTime() {
        var seconeds = Math.round(videoNative.currentTime);
    ...
    function toggleMute() {
        if (!videoNative.muted) {
            videoNative.muted = true;
            volume.css({
                'background': 'url(/etc.clientlibs/incredible-india-v2/components/clientlibs/main/resources/video/mute.svg)',
                'background-size': '100% 100%'
            });
            intensity.hide();
        } else {
            ...
            volume.css({
                'background': 'url(/etc.clientlibs/incredible-india-v2/components/clientlibs/main/resources/video/volume.svg)',
                'background-size': '100% 100%'
            });
            intensity.show();
        }
    ...
    function changeVolume(event) {
        ...
        videoNative.volume = (mouseX / width);
    ...
    function create(itemList) {
        ...
        div.setAttribute("class", "video pointer");
        div.setAttribute("data-name", itemList.name);

        imageContainer.setAttribute("class", "video-image v-image-hero")
        img.setAttribute("src", itemList.img+"/jcr:content/renditions/cq5dam.web.256.256.jpeg");
    ...
    function loadList(playlist) {
        ...
        setTimeout(function () {
            var fId = $('.video:first-child').hasClass('playing'),
                lId = $('.video:last-child').hasClass('playing'),
                firstVideo= $(playList).find('.video:first-child');
            $(firstVideo).click();
            if (fId) {
                $(vbtnPrev).prop("disabled", true);
                $(vbtnNext).prop("disabled", false);
    ...
    function getVideos() {
        var videoList=[];
         var locations = $(".video-data div");           
             $.each(locations, function(index, locaton) {
                videoList.push({
                    id:$(locaton).data("id"),
                    name:$(locaton).data("name"),
                    source:$(locaton).data("source"),
                    duration:$(locaton).data("duration"),
                    img:$(locaton).data("img"),
                });
         });
            ...
            if (videos.length > 0) {
                ...
                $(container).addClass('show-play-list');
            } else {
                $(container).removeClass('show-play-list');
            }
        // });
    }
...
// });
 ...
 //$(document).ready(function () {
      $('.abc').on('click', function () {
        $('#customvideoModal').modal();
      });
      $('#customvideoModal').on('shown.bs.modal', function () {
        //initializes the plugin with empty options
        $($("#customvideoModal").detach()).appendTo("body"); 
        initializeVideoJS();
        setTimeout(function () {
          $('#playlist').overlayScrollbars({
 		...
 		$('#customvideoModal').on('hide.bs.modal', function () {
     ...
     $('#modal-repository').on('show.bs.modal', function(e) {
